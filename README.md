...
│       ├── 比賽報名表.odt           # 模擬競賽報名場景
│       ├── 退學申請單.odt           # 模擬學校申請場景
│       └── ODF決賽_工具審核文件.odt  # 執行後自我審核表範本，供人工複核遮蔽結果用
...
`tests/samples/ODF決賽_工具審核文件.odt` 是一份執行流程自我審核表，用於工具跑完之後的人工複核紀錄，
可搭配 `outputs/<檔名>_report.odt` 的偵測結果比對確認，記錄正則層/Claude 層各找到幾筆、合併後總筆數、
自我驗證輪數是否正常通過。
